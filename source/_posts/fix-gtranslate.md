...
- 若要检测IPV6地址，需在目录下打开`cmd`，运行
  ...
  ```
检测完会列出可用的IP地址，并以延迟最低的IP给你列好填写规则
...
在解压后得到的文件夹下打开终端，运行
```bash
sudo chmod +x GoogleTranslateIpCheck
```
来为程序添加执行权限，然后运行
...
检测完后会列出可用的IP地址，并以延迟最低的IP给你列好填写规则

此时打开`/etc/hosts`文件，在末尾加入给我们的填写规则，并保存

### Android（需ROOT）

需借助[Termux](https://github.com/termux/termux-app/releases/latest)，且需使用项目中的`linux-arm64`包，其他与[Linux](#linux)一致
...
- 检测的IPV4基本都不可用，若你的网络支持IPV6，强烈建议使用检测IPV6的方式
