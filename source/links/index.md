---
title: 友情链接
date: 2026-01-01 13:51:39
top_img: transparent
type: 'link'
flink_url: 'http://ftp.oxue.de/json/blog/links.json5'
random: true
---

{% note info %}
该页面友链信息来自[外部链接](http://ftp.oxue.de/json/blog/links.json5)，可能因某些原因而加载缓慢，还请稍安勿躁…
{% endnote %}

## 意见互换

如果您觉得本站值得与贵站互换友链，在留下贵站信息前务必阅读以下注意事项📋：
1. 贵站内容不涉黄、赌、毒，不违背人道主义、公序良俗
2. 贵站内容以非纯收集类、非有关中国大陆境内公民私自接入国际互联网工具为主
3. 贵站支持HTTPS访问
4. 贵站需维持友链状态，如有变更或删除意见也可以在评论区留言或给我邮件

另外，如果贵站长时间无法访问，贵站友链信息会在[友链文件](http://ftp.oxue.de/json/blog/links.json5)中注释掉，直至贵站站点恢复正常状态

以下是本站友链互换模板：
```json
"name": "站点名称",
"link": "站点链接",
"avatar": "站点图标",
"descr": "站点描述"
```
劳驾您按照模板在评论区留下贵站信息，或者[给我邮件](mailto:msbyshens@outlook.com)，站长**如有空闲**定会快马加鞭查阅并添加贵站信息至本页👀

以下是本站信息：
- **站点名称**：`傲雪の`
- **站点链接**：`https://www.oxue.de`
- **站点图标**：`https://av.oxue.de/155360005`
- **站点描述**：`无聊的一天搭配神经的我～`
- **站点快照**：`https://images.oxue.de/file/siteshot.webp`

请您在贵站**按需**参照如上信息添加本站❤️

<p align="center"><strong>永远欢迎您到本站浏览！</strong></p>
